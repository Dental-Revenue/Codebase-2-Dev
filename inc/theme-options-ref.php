...
/**
 * Return the Google Map picture in footer style B.
 * 
 * @echo boolean  $echo
 * @id   boolean  $id
 * 
 * @return file     Returns the Google Map picture
 */
function googleMapPicture($echo = true, $id = false)
{
    $option = get_option('practice_info'); 
    if($id){
        if(!empty($option['google_map_picture'])){if($echo){echo $option['google_map_picture_id'];}else{return $option['google_map_picture_id'];}}else{if($echo){echo '';}else{return '';}}
    } else {
        if(!empty($option['google_map_picture'])){if($echo){echo $option['google_map_picture'];}else{return $option['google_map_picture'];}}else{if($echo){echo get_template_directory_uri().'/assets/images/layout/Default-Google-Map-Picture.png';}else{return get_template_directory_uri().'/assets/images/layout/Default-Google-Map-Picture.png';}}
    }
}
/**
 * Return the Google Map URL for footer style B
 *
 * @echo boolean $echo
 * 
 * @return text
 */
function googleMapUrl($echo = true)
{
    $option = get_option('practice_info');
    if (!empty($option['google_map_url'])) {
        if ($echo) {
            echo $option['google_map_url'];
        } else {
            return $option['google_map_url'];
        }
    } else {
        if ($echo) {
            echo '';
