	...
	<div role="main" class="service-template">	
		<div class="row">
			...
			<div class="columns eight">
              ...
              <div class="service-testimonial"><?php echo wp_trim_words(apply_filters('get_the_content', $testimonial->post_content),50); ?><span><?php echo get_the_title($id); ?></span></div>
      	  	<?php } ?>
				  </div>
				<?php } ?>
	...
	<?php if(get_post_meta($postID, 'service_related_hidden', true)=='false'){ ?>
	  <div class="service-related service-module">
        ...
        foreach ((array)$related as $key => $entry ) { ?>
          <a href="<?php echo get_permalink($entry); ?>" class="btn"><?php echo get_the_title($entry); ?></a>
        <?php } ?>
    	</div>
	  </div>
	<?php } ?>
