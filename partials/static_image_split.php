...
<div class="static_image_split">    
    ...
    ?>
  <div class="static_image_split-img <?php if ($embedtype != 'image') {
        echo 'popup-';
                                     } ?><?php echo $embedtype ?><?php if ($embedtype === 'custom') {
                                        echo '-' . $randomnumber;
                                     } ?> <?php if ($embedtype != 'image') {
                                        echo 'embed-button-' . $embedicon . ' static_image_split-img-btn-bg';
                                     } ?>" <?php if ($embedtype === 'youtube' || $embedtype === 'iframe') {
                                        echo 'href="' . $embed . '"';
                                     } ?> style="<?php if ($side_image_left_right === 'left') {
                                        echo "";
                                     } else {
                                         echo "left: 50%;";
                                     } ?>">
  <img alt="<?php if (!empty($image_alt)) {
        echo $image_alt;
            } else {
                echo 'Background Image';
            } ?>" src="<?php echo $image_xxl[0]; ?>" srcset="<?php echo $image_lg[0]; ?> 500w, <?php echo $image_xl[0]; ?> 700w, <?php echo $image_xxl[0]; ?> 3000w" sizes="100vw,(min-width: 300px) 700px,(min-width: 700px) 1300px" />
  </div>
  <div class="static_image_split-right" style="<?php if ($side_image_left_right === 'left') {
        echo "margin-left:50%;";
                                               } else {
                                                    echo "margin-left:0;";
                                               } ?>">
    <div class="static_image_split-content" style="text-align:<?php echo $text_align; ?>; padding: 5% <?php echo $text_padding; ?>% 7% <?php echo $text_padding; ?>%;">
        <?php if (!empty($headline)) {
            ?><h2 style="text-align:<?php echo $text_align; ?>" class="<?php echo $headline_style; ?>"><?php echo $headline; ?></h2>
        <?php } ?>
        <?php if ($display_form === 'no') { ?>
            <?php if (!empty($content)) { ?>
                <div class="static_image_split-paragraph"><?php echo $content; ?></div>
            <?php } ?>
            ...
            <div class="static_image_split-contact">
                <?php if ($display_phone === 'yes' || $display_address === 'yes') {?>
                    ...
                    <?php if ($display_phone === 'yes') {?>
                        <a class="contact-option" <?php $option = get_option('practice_info'); if (!empty($option['new_patient_phone'])) {
                            echo 'href="tel:' . $option['new_patient_phone'] . '"';
                       } ?>>
            <i class="icon fas fa-phone"></i>
            <p>New Patients <span class="bold tracknum"><?php site_ops_new_patient_phone(); ?></span></p>
                    <?php } ?>
                    <?php if ($display_address === 'yes') {?>
        <a class="contact-option street-address" <?php $option = get_option('practice_info'); if (!empty($option['google_place_id'])) {
            echo 'href="https://www.google.com/maps/search/?api=1&query=Google&query_place_id=' . $option['google_place_id'] . '" target="_blank"';
       } ?>>
    ...
    <a href="<?php echo $btn_url; ?>" class="btn solid" <?php if ($display_phone === 'no' && $display_address === 'no') {?>
        style="margin-top: 20px;"
   <?php } ?>><?php echo $btn_text; ?></a>
                <?php } ?>
    </div>
            <?php } ?>
        <?php } else { ?>
