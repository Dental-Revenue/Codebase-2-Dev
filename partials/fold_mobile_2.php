...
<div>
    <?php if ($fold_image != '') { ?>
        <img src="<?php echo $fold_image; ?>" alt="
            <?php if ($fold_image_alt !='') {
                echo $fold_image_alt;
            } else { ?>
                Background Image
            <?php } ?>" class="static-module-image" />
    <?php } ?>
    <?php if ($color != '') { ?>
        <span class="fold-mobile-color-overlay" style="<?php echo $colorCSS; ?>"></span>
    <?php } ?>
    <div class="fold-mobile-slide">
        <div class="fold-overlay" style="
            <?php if ($color != '') {
                echo $colorCSS;
            } ?>
            <?php echo $colorDarkness; ?>">
        </div>
        <div class="fold-mobile-slide-text">
            <?php if ($fold_title) : ?>
                <p><?php echo $fold_title; ?></p>
            <?php endif; ?>
            <?php if (isset($fold_excerpt) && $fold_excerpt != '') { ?>
                <p><?php echo $fold_excerpt; ?></p>
            <?php } ?>
            <?php if (isset($cta_text) && $cta_text != '') { ?>
                <a class="btn solid" href="<?php echo $cta_url; ?>"><?php echo $cta_text; ?></a>
            <?php } ?>
            <?php if (isset($cta_text_2) && $cta_text_2 != '') { ?>
                <a class="btn solid" href="<?php echo $cta_url_2; ?>"><?php echo $cta_text_2; ?></a>
            <?php } ?>
        </div>
    </div>
</div>